...
1. **Top Row KPI Cards (Card (New) Visual - NEW)**:
   - **Business Value**: Solves **"How efficient is passenger planning behavior, and what is our proportional booking success vs cancellation leakage?"** (tracks ratios and lead times instead of duplicating the raw status counts already shown in the charts below).
   - **Visualization Pane**: Select **Card (new)** icon.
   - **Coordinates**: X = `15px`, Y = `85px`, Width = `1250px`, Height = `120px`.
   - **Field Wells Configuration**:
     - Drag `_Measures[Avg Lead Time]`, `_Measures[Confirmation Rate]`, `_Measures[Cancellation Rate]`, and `_Measures[Pending Rate]` into the Values box.
   - **Visual Layout & Formatting**:
     - Orientation: **`Horizontal`**, Columns: **`4`**, Card spacing: `15px`, Vertical alignment: **`Middle`**.
     - Style backgrounds, fonts, and borders consistent with theme. Upload matching icons (e.g., `icon_lead_time.png` for lead time, `icon_confirmed.png` for confirmation rate, `icon_cancellation_rate.png` for cancellation rate, and `icon_pending.png` for pending rate). Set size to **`50px`**.
